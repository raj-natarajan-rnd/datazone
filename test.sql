-- Optional: set the process step for this session
SELECT set_config('acme.process_step', '1-Initial', true);

-- 1) Housing: flip 3 audited columns → expect 3 audit rows
...
-- 2) Population: flip 4 audited columns → expect 4 audit rows
...
-- 3) Roster: change 3 audited columns → expect 3 audit rows
